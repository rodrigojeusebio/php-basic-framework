<div class="p-6 bg-white shadow rounded-lg">
    <h1 class="text-2xl font-bold text-gray-800 mb-4">
        Task: <?  echo htmlspecialchars($task->description);  ?>
    </h1>
    <p class="text-lg text-gray-600">
        Status:
        <span class="<?= $task->complete ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold' ?>">
            <?  echo htmlspecialchars($task->complete ? 'Complete' : 'Not complete');  ?>
        </span>
    </p>
    ...
    <div class="flex space-x-4 mt-4">
        ...
        <!-- Edit Button -->
        <a href="/tasks/<?  echo htmlspecialchars($task->id);  ?>/edit">
        ...
        <!-- Delete Button -->
        <form action="/tasks/<?  echo htmlspecialchars($task->id);  ?>" method="POST"
